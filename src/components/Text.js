import { FontLoader, Vector3, TextGeometry } from "three"
...
function Text({ children, size = 1, left, right, top, bottom, color = "white", opacity = 1, height = 0.01, layers = 0, font = "/MOONGET_Heavy.blob", ...props }) {
  ...
  const geom = usePromise(() => new Promise(res => res(new TextGeometry(children, { font: data, size: 1, height, curveSegments: 32 }))), [children])
